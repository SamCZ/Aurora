...
namespace Aurora
{
	...
	static inline int SafeRightShift(int32_t left, uint8_t right)
	{
	    if(left >= 0) return static_cast<uint32_t>(left) >> right;

	    uint32_t inverted = -left;

	    return -static_cast<int32_t>(inverted >> right) - 1;
	}

    static inline int SafeLeftShift(int32_t left, uint8_t right)
    {
        if(left >= 0) return static_cast<uint32_t>(left) << right;
    ...
    static glm::vec<N, int32_t, glm::defaultp> SafeRightShift(const glm::vec<N, T, glm::defaultp>& left, uint8_t right)
    {
        glm::vec<N, int32_t, glm::defaultp> out;

        for (int i = 0; i < N; ++i) {
            out[i] = SafeRightShift((int32_t)left[i], right);
        }
    ...
    template<typename T, size_t N>
    static glm::vec<N, int32_t, glm::defaultp> SafeLeftShift(const glm::vec<N, T, glm::defaultp>& left, uint8_t right)
    {
        glm::vec<N, int32_t, glm::defaultp> out;

        for (int i = 0; i < N; ++i) {
            out[i] = SafeLeftShift((int32_t)left[i], right);
        }
...
namespace glm
{
    ...
    {
        return SmoothDamp(current, target, currentVelocity, smoothTime, Aurora::DoubleMax, deltaTime);
    }
