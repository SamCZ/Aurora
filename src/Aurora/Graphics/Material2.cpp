...
namespace Aurora
{
	Material2::Material2(String name, const Path &shaderPath, ShaderMacros_t macros)
	...
	Material2::Material2(String name, ShaderMacros_t macros)
	: m_Name(std::move(name)),
	  m_Macros(std::move(macros)),
	  m_CurrentPipelineState(nullptr),
	  m_CurrentResourceBinding(nullptr),
		...
		for (int i = 0; i < shader->GetResourceCount(); ++i) {
			...
			switch (desc.Type) {
				case SHADER_RESOURCE_TYPE_CONSTANT_BUFFER:
					LoadConstantBuffers(shaderObj, desc, barriers);
					hasConstantBuffers = true;
					break;
				case SHADER_RESOURCE_TYPE_TEXTURE_SRV:
					LoadTextureSRV(shaderObj, desc, barriers);
					hasTextureSRVs = true;
					break;
	...
	void Material2::LoadConstantBuffers(ShaderObject &object, ShaderResourceDesc desc, std::vector<StateTransitionDesc>& barriers)
	{
		...
		ConstantBufferList& constantBufferStorage = m_ShaderConstantBuffers[shaderType];
	...
	void Material2::LoadTextureSRV(ShaderObject &object, ShaderResourceDesc desc, std::vector<StateTransitionDesc>& barriers)
	{
		...
		samplerList.emplace_back(shaderType, desc.Name, SamLinearClampDesc);
	}

	void Material2::DeleteConstantBuffers(const SHADER_TYPE& type)
	{
		m_ShaderConstantBuffers.erase(type);
	}

	void Material2::DeleteTextureSRVs(const SHADER_TYPE &type)
	{
		m_ShaderTextures.erase(type);
		m_ShaderSamplers.erase(type);
	}

	void Material2::ApplyShaderToPSO(IShader* shader, const SHADER_TYPE& shaderType)
	{
		switch (shaderType) {
	...
	void Material2::ValidateGraphicsPipelineState()
	{
		// Setup PSO vars
		if(m_NeedsRebuildResourceLayout) {
			...
			for(const auto& it : m_ShaderSamplers) {
				for(auto sampler : it.second) {
					if(std::find(usedVars.begin(), usedVars.end(), sampler.SamplerOrTextureName) == usedVars.end()) {
						m_PSO_ShaderResourceSamplers.push_back(sampler);
					} else {
	...
	uint32_t Material2::HashPSO(const GraphicsPipelineStateCreateInfo &gpsci)
	{
		CrcHash hasher;
	...
	void Material2::SetSampler(const String &textureName, const SamplerDesc &sampler)
	{
		for (auto& samplerList : m_ShaderSamplers) {
			for(auto& samplerFromList : samplerList.second) {
				if(textureName == samplerFromList.SamplerOrTextureName) {
					samplerFromList.Desc = sampler;
					m_NeedsRebuildResourceLayout = true;
				}
			}
		}
	...
	void Material2::SetTexture(const String &name, const RefCntAutoPtr<ITexture> &texture)
	{
		if(texture == nullptr) {
